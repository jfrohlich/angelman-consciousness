function [B,stats,CVpart] = mylassoglm(x,y,distr,varargin)

% This is the native lassoglm function modified by JF on 07/29/21 to also
% return the cross-validation partion 
...
%
%     'Balanced'       Unique IDs showing which data came from which
%                      subjects. Use this for balanced datasets so that all
%                      data from one subject is on same side of CV
%                      partition
%
...
pnames = { 'alpha' 'numlambda' 'lambdaratio' 'lambda' ...
    ...
    'predictornames' 'options' 'balanced' };
dflts  = {  1       100       LRdefault     []      ...
     []      true          1e-4    'resubstitution'  1  1e4 ...
     {}               []        []};
[alpha, nLambda, lambdaRatio, lambda, ...
    dfmax, standardize, reltol, cvp, mcreps, maxIter, predictorNames, parallelOptions,balanced] ...
...
internal.stats.checkSupportedNumeric('Weights',pwts,false,false,false,false);
...
[X,Y,pwts,nLambda,lambda,dfmax,cvp,mcreps,predictorNames,ever_active] = ...
    ...
    standardize,reltol,cvp,mcreps,predictorNames,balanced);
...
if ~isequal(cvp,'resubstitution')
    ...
    if ~isempty(balanced)
        mytraining = balanced(:,:,1);
        mytest = balanced(:,:,2);
        CVpart.training = mytraining;
        CVpart.test = mytest;
        cvDeviance = mycrossval(cvfun,[weights(:) offset(:) X],response, ...
            'Partition',cvp,'Mcreps',mcreps,'Options',parallelOptions,...
            'mytraining',mytraining,'mytest',mytest);
    else
        CVpart.training = nan(size(cvp.training,1),cvp.NumTestSets);
        CVpart.test = nan(size(cvp.test,1),cvp.NumTestSets);
        for i = 1:cvp.NumTestSets
            CVpart.training(:,i) = cvp.training(i);
            CVpart.test(:,i) = cvp.test(i);
        end
        cvDeviance = mycrossval(cvfun,[weights(:) offset(:) X],response, ...
            'Partition',cvp,'Mcreps',mcreps,'Options',parallelOptions);
    end
...
if standardize
    ...
    switch regressionType
        ...
        case 'GLM'
            ...
            sigmaX = 1;
            muX = zeros(1,size(X,2),dataClass);
    end
end
...
switch regressionType
    case 'OLS'
        Intercept = muY-muX*B;
    case 'GLM'
        Intercept = zeros(1,length(lambda),dataClass);
...
switch regressionType
    case 'OLS'
        ...
        varargout{1} = mspe;
    case 'GLM'
        deviance = zeros(1,length(lambda),dataClass);
        for i=1:length(lambda)
...
[b,active] = ...
    internal.stats.lassoglmCoordDescentCycleNoRecalc(X0,wX0,wX2,Y0,b,active',shrinkFactor,threshold); 
active = active';
...
function [b,varargout] = ...
    ...
    wX2 = zeros(P,1,'like',X);
    wX2 = internal.stats.lassoglmWtimesX2(X,weights',wX2,active');    
...
% have been able to converge to a reasonable estimate.  If so, the inputs
