function [X,Y,T,auc,optrocpt,bsdis_auc,subY,subYnames] = ...
    myperfcurve(cls,scores,posClass,varargin)
%PERFCURVE Compute Receiver Operating Characteristic (ROC) curve or other
%   performance curve for classifier output.

%   % This is the version of the native matlab function edited by JF to
%   return the bootstrapped resamples as the 6th output argument. 
%  
...
%   matrix, strings, cell array of strings or categorical vector (see help for
...
%   logical vectors, character matrices, strings, cell arrays of strings or
...
%                  vector, logical scalar, character matrix, strings, cell
%                  array of strings or categorical vector. By default,
%                  NegClass is set to 'all' and all classes found in the
%                  input array of labels that are not the positive class
%                  are considered negative. If NegClass is a subset of the
%                  classes found in the input array of labels, observations
%                  with labels that do not belong to either positive or
%                  negative classes are discarded.
%  
...
if nargin > 0
    cls = convertStringsToChars(cls);
end

if nargin > 2
    posClass = convertStringsToChars(posClass);
end
...
if docv || doboot
    ...
    elseif doboot
        ...
        [ci, bsdis] = bootci(nboot,{bootfun (1:size(W,1))'},...
...
if nargout>3
    ...
    if docv || doboot
        ...
        elseif doboot
            ...
            [ciauc, bsdis_auc] = bootci(nboot,{bootfun (1:size(W,1))'},...
                'weights',sum(W,2),'alpha',alpha,'type',boottype,'options',paropts,...
                bootarg{:});
            ciauc = ciauc';
        end
