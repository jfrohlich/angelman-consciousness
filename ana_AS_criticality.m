...
switch sleep
    case true
        DIRRESULT = './2021_analysis/AS_output/CRIT/sleep/';
        if ~exist(DIRRESULT), mkdir(DIRRESULT), end
    case false
         DIRRESULT = './2021_analysis/AS_output/CRIT/wake';
        if ~exist(DIRRESULT), mkdir(DIRRESULT), end
end
...
for ifile = startndx:length(files)
    ...
    switch sleep
        ...
        case false
            badart = data.cfg.dattype.SLEEP_MONTI; % exclude anything that could possiblly be sleep
            % SLEEP_MONTI + Drowsy = sleep (old field), we exclude both
    end
    
    % Set bad data to NaNs for purpose of taking freq transform
    % Drowsy is bad whether we're dealing with sleep or wake
    ...
    cfg.method = 'minmax';
    Cr = nan(size(datamat,1),size(datamat,2)-n_win+1); % preallocation
    valid = nan(size(datamat,1),size(datamat,2)-n_win+1); % preallocation
    K = nan(size(datamat,1),size(datamat,2)-n_win+1); % preallocation
    FLP = nan(size(datamat,1),size(datamat,2)-n_win+1); % preallocation
    ...
    for isection = 1:n_shift:size(datamat,2)-n_win+1
        for ich = 1:size(datamat,1)
            ...
            if all(~isnan(section)) && ~all(section==0) % if there are no nans and the data are not just zeros
                ...
                FLP(ich,cnt) = f_lp; 
                if ich == 1
                    fprintf('%2.1f%% complete\n',cnt/(durgood/win_len)*100)
                end
            else
                %fprintf('Data are bad (NaNs or 0s)\n')
