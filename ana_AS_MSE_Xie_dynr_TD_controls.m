...
switch sleep
    case true   
        DIRRESULT = './2021_analysis/TD_output/Xie/r=0.15/dynr/sleep/'; if ~exist(DIRRESULT), mkdir(DIRRESULT), end
        ntrl = 2; 
    case false
        DIRRESULT = './2021_analysis/TD_output/Xie/r=0.15/dynr/wake/'; if ~exist(DIRRESULT), mkdir(DIRRESULT), end
        ntrl = 1;
end
...
%            || logical(sum(ismember(select_files,sprintf('%s%s',files(ifile).name(1:end-4),'_wake_MSE.mat'))))
        ...
        for isection = 1:n_shift:size(datamat,2)-n_win+1
            ...
            if ~any(isnan(section(:))) % if the data aren't just nans
                if surrogate
